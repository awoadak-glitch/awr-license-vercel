...
const PROTOCOL = "oscar-verify-v1";
const ALLOWED_PACKAGE = "com.drama.mp4";
const ALLOWED_CERT_SHA256 =
...
function deriveSigningMaterial() {
  ...
  for (let counter = 0; counter < 32; counter++) {
    const d = createHmac("sha256", pepper)
      .update(`${PROTOCOL}:signing:${counter}`)
      .digest();
...
function fail(code, status = 200) {
  return json({ success: false, code, protocol: PROTOCOL }, status);
}
...
export default {
  async fetch(request) {
    ...
    try {
      ...
      if (packageName !== ALLOWED_PACKAGE) return fail("PACKAGE_NOT_ALLOWED", 403);
      if (!/^[a-f0-9]{64}$/.test(certSha256)) return fail("CERT_REQUIRED", 400);
      if (certSha256 !== ALLOWED_CERT_SHA256) return fail("CERT_NOT_ALLOWED", 403);
